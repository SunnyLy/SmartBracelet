...
public class IconRoundCornerProgressBar extends LinearLayout {
    ...
    private void setup(Context context, AttributeSet attrs) {
        ...
        if (!isIconSetBeforeDraw) {
            int iconResource = typedArray.getResourceId(
                    R.styleable.RoundCornerProgress_rcIconSrc,
        ...
        iconSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                iconSize, metrics);
        iconSize = (int) typedArray.getDimension(
                R.styleable.RoundCornerProgress_rcIconSize, iconSize);
        ...
        iconPadding = (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, iconPadding, metrics);
        iconPadding = (int) typedArray.getDimension(
                R.styleable.RoundCornerProgress_rcIconPadding, iconPadding);
        layoutHeader.setPadding(iconPadding, iconPadding, iconPadding,
                iconPadding);
        if (!isHeaderColorSetBeforeDraw) {
            setHeaderColor(typedArray.getColor(
                    R.styleable.RoundCornerProgress_rcHeaderColor, headerColor));
        }
        ViewTreeObserver observer = layoutHeader.getViewTreeObserver();
        observer.addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
            @Override
            public void onGlobalLayout() {
                layoutHeader.getViewTreeObserver()
    ...
    private void setProgressColor(ViewGroup layout, int color) {
        ...
        gradient.setCornerRadii(new float[]{0, 0, radius, radius, radius,
                radius, 0, 0});
    ...
    public void setHeaderColor(int color) {
        ...
        gradient.setCornerRadii(new float[]{radius, radius, 0, 0, 0, 0,
                radius, radius});
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
